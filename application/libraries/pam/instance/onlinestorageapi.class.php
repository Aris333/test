...
class OnlineStorageApi extends ApiClient
{
	...
	*/
	public function ListProviders($attach_settings = false)
	{
		return $this->CallApi('OnlineStorage/ListProviders',array('attach_settings' => $attach_settings),false);
	}
	...
	*/
	public function Authenticate($provider, $username, $password)
	{
		return $this->CallApi('OnlineStorage/Authenticate',array('provider' => $provider, 'username' => $username, 'password' => $password),false);
	}
	...
	*/
	public function SetAuthToken($provider, $token, $username = false)
	{
		return $this->CallApi('OnlineStorage/SetAuthToken',array('provider' => $provider, 'token' => $token, 'username' => $username),false);
	}
	...
	*/
	public function DropAuthentication($provider)
	{
		return $this->CallApi('OnlineStorage/DropAuthentication',array('provider' => $provider),false);
	}
	...
	*/
	public function GetProviderLogo($provider, $size)
	{
		return $this->CallApi('OnlineStorage/GetProviderLogo',array('provider' => $provider, 'size' => $size),true);
	}
	...
	*/
	public function ListFolderContents($provider, $folder = false, $clear_cache = false)
	{
		return $this->CallApi('OnlineStorage/ListFolderContents',array('provider' => $provider, 'folder' => $folder, 'clear_cache' => $clear_cache),false);
	}
	...
	* @param string $provider Provider name.
	*/
	public function CheckProviderState($provider)
	{
		return $this->CallApi('OnlineStorage/CheckProviderState',array('provider' => $provider),false);
	}
}
