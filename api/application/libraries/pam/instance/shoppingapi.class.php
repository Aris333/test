...
class ShoppingApi extends ApiClient
{
	...
	*/
	...
	{
		return $this->CallApi('Shopping/MakeOrder',array(),false);
	}
	...
	*/
	public function ListFaxInCountries($culture = '')
	{
		return $this->CallApi('Shopping/ListFaxInCountries',array('culture' => $culture),false);
	}
	...
	*/
	...
	{
		return $this->CallApi('Shopping/ListFaxInAreacodes',array('country_code' => $country_code, 'state' => $state),false);
	}
	...
	* @param $nationality required to be submitted for South Africa and Italy
	* @param $identityDocumentType required to be submitted for some countries, like South Africa and Italy
	* @param $identityDocumentNumber required to be submitted for some countries, like South Africa and Italy
	* @param $issuingAuthority required to be submitted for some countries, like Italy
	* @param $issuingDate required to be submitted for some countries, like Italy
	* @param $proofOfId required to be submitted for some countries, like South Africa and Italy
	*/
	public function CheckAddressForFaxInNumber($group_id, $salutation, $country_code_a3, $city, $zip_code, $street_name, $building_number, $building_letter = '', $company_name = '', $first_name = '', $last_name = '', $verify_doc_uuid = '', $token = '', $rnd = '', $trial_request = '', $nationality = '', $identityDocumentType = '', $identityDocumentNumber = '', $issuingAuthority = '', $issuingDate = '', $proofOfId = '')
	{
		return $this->CallApi('Shopping/CheckAddressForFaxInNumber',array('group_id' => $group_id, 'salutation' => $salutation, 'country_code_a3' => $country_code_a3, 'city' => $city, 'zip_code' => $zip_code, 'street_name' => $street_name, 'building_number' => $building_number, 'building_letter' => $building_letter, 'company_name' => $company_name, 'first_name' => $first_name, 'last_name' => $last_name, 'verify_doc_uuid' => $verify_doc_uuid, 'token' => $token, 'rnd' => $rnd, 'trial_request' => $trial_request, 'nationality' => $nationality, 'identityDocumentType' => $identityDocumentType, 'identityDocumentNumber' => $identityDocumentNumber, 'issuingAuthority' => $issuingAuthority, 'issuingDate' => $issuingDate, 'proofOfId' => $proofOfId),false);
	}
	...
	*/
	public function RequestTrialNumber($group_id, $address_uuid, $token, $rnd)
	{
		return $this->CallApi('Shopping/RequestTrialNumber',array('group_id' => $group_id, 'address_uuid' => $address_uuid, 'token' => $token, 'rnd' => $rnd),false);
	}
	...
	*/
	...
	{
		return $this->CallApi('Shopping/GetNearestFaxInNumber',array('ip_address' => $ip_address),false);
	}
	...
	*/
	public function GetNumberPrice($plan_type, $months = '1')
	{
		return $this->CallApi('Shopping/GetNumberPrice',array('plan_type' => $plan_type, 'months' => $months),false);
	}
	...
	* @param string $number current active fax in number
	*/
	public function GetExtendNumberCost($number)
	{
		return $this->CallApi('Shopping/GetExtendNumberCost',array('number' => $number),false);
	}
	...
	*/
	public function ListAvailablePoolNumbers($country_code, $prefix = '', $us_state = '', $group_id = '', $limit = '10')
	{
		return $this->CallApi('Shopping/ListAvailablePoolNumbers',array('country_code' => $country_code, 'prefix' => $prefix, 'us_state' => $us_state, 'group_id' => $group_id, 'limit' => $limit),false);
	}
	...
	*/
	public function isPIRequiredForGroup($group_id)
	{
		return $this->CallApi('Shopping/isPIRequiredForGroup',array('group_id' => $group_id),false);
	}
	...
	*/
	public function ChangeVerifyDoc($number, $file_size, $file_contentmd5)
	{
		return $this->CallApi('Shopping/ChangeVerifyDoc',array('number' => $number, 'file_size' => $file_size, 'file_contentmd5' => $file_contentmd5),false);
	}
	...
	*/
	public function UploadVerifyDoc($file_size = '', $file_content_md5 = '')
	{
		return $this->CallApi('Shopping/UploadVerifyDoc',array('file_size' => $file_size, 'file_content_md5' => $file_content_md5),false);
	}
	...
	*/
	public function UpgradeBasic2ProForPFCredit($number, $only_calculate = '1')
	{
		return $this->CallApi('Shopping/UpgradeBasic2ProForPFCredit',array('number' => $number, 'only_calculate' => $only_calculate),false);
	}
	...
	*/
	public function OrderNumberForPFCredit($area_code_id, $name, $address, $zip, $city, $country_code, $plan_type, $state_code = '', $company = '', $only_validate = '1', $months = '1', $pool_id = '', $verify_doc_uuid = '')
	{
		return $this->CallApi('Shopping/OrderNumberForPFCredit',array('area_code_id' => $area_code_id, 'name' => $name, 'address' => $address, 'zip' => $zip, 'city' => $city, 'country_code' => $country_code, 'plan_type' => $plan_type, 'state_code' => $state_code, 'company' => $company, 'only_validate' => $only_validate, 'months' => $months, 'pool_id' => $pool_id, 'verify_doc_uuid' => $verify_doc_uuid),false);
	}
	...
	* @param int $months 1..12 months in plan
	*/
	public function ExtendNumberForPFCredit($number, $name, $address, $zip, $city, $country_code, $plan_type, $state_code = '', $company = '', $only_validate = '1', $months = '1')
	{
		return $this->CallApi('Shopping/ExtendNumberForPFCredit',array('number' => $number, 'name' => $name, 'address' => $address, 'zip' => $zip, 'city' => $city, 'country_code' => $country_code, 'plan_type' => $plan_type, 'state_code' => $state_code, 'company' => $company, 'only_validate' => $only_validate, 'months' => $months),false);
	}

	/**
	* Returns all available items for sale for PamFax credit. Now supported only Plans with FaxIn numbers.
	*/
	public function ListAvailableItemsForPFCredit()
	{
		return $this->CallApi('Shopping/ListAvailableItemsForPFCredit',array(),false);
	}
	...
	* @return 
	*/
	public function UploadVerifyDocFromPortal($token)
	{
		return $this->CallApi('Shopping/UploadVerifyDocFromPortal',array('token' => $token),false);
	}
}
