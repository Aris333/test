...
class FaxHistoryApi extends ApiClient
{
	...
	*/
	public function ListSentFaxes($current_page = 1, $items_per_page = 20, $array_for_api = false)
	{
	...
	* @param boolean $no_trash fix process of deleteing: 1)to move to trash (false) or 2)to delete without adding to trash (true).
	*/
	public function DeleteFaxesForPeriod($type, $date1, $date2, $no_trash = false)
	{
		return $this->CallApi('FaxHistory/DeleteFaxesForPeriod',array('type' => $type, 'date1' => $date1, 'date2' => $date2, 'no_trash' => $no_trash),false);
	}
	...
	*/
	public function DeleteFromListRecentRecipients($number)
	{
		return $this->CallApi('FaxHistory/DeleteFromListRecentRecipients',array('number' => $number),false);
	}
